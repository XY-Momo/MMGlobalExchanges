 ...
 * 个人出售记录
 */
public class SellRecord extends Chest {

    // 存储玩家上一次的翻页记录
    private final Map<String, Integer> playerPages = new HashMap<>();
    private PlayerData playerData;

    public SellRecord(String title) {
        super(title, 54);
    }
    ...
    private void init(Player player) {
        ...
        int max_item = MMGlobalExchanges.instance.getConfig().getInt("market_personal_sell_max");
        this.addReturn();
        if (page == 1) {
            this.removeItem(47);
            this.removeItem(48);
    ...
    private void loadItems(Player player) {
        ...
        for (int id : playerData.getSellItems()) {
            if (index / MMGlobalExchanges.instance.getConfig().getInt("market_personal_sell_max") == page - 1) {
                int slot = index % MMGlobalExchanges.instance.getConfig().getInt("market_personal_sell_max");
    ...
    public int getSellNum() {
        return playerData.getSellItems().size();
    }
    ...
    private void addInfo(Player player) {
        ...
        lore.add(String.valueOf((playerData.getSellItems().size() - 1) / MMGlobalExchanges.instance.getConfig().getInt("market_personal_sell_max") + 1));
        itemMeta.setLore(lore);
        item.setItemMeta(itemMeta);
        addItem(52, item);
    }
