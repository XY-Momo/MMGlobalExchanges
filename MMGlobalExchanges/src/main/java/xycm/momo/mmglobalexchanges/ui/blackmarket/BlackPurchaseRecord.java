 ...
 * 个人购买记录
 */
public class BlackPurchaseRecord extends Chest {

    // 存储玩家上一次翻页记录
    private final Map<String, Integer> playerPages = new HashMap<>();
    private PlayerData playerData;

    public BlackPurchaseRecord(String title) {
        super(title, 54);
    }
    ...
    private void init(Player player) {
        ...
        int max_item = MMGlobalExchanges.instance.getConfig().getInt("black_market_personal_purchase_max");
        this.addReturn();
        if (page == 1) {
            this.removeItem(47);
            this.removeItem(48);
            this.removeItem(50);
            this.removeItem(51);
            if (playerData.getBlackBuyItems().size() > max_item) {
                this.addNextPage();
                this.addLastPage();
            }
        } else if (page < (playerData.getBlackBuyItems().size() - 1) / max_item + 1) {
            this.addFirstPage();
    ...
    private void loadItems(Player player) {
        ...
        for (int id : playerData.getBlackBuyItems()) {
            if (index / MMGlobalExchanges.instance.getConfig().getInt("black_market_personal_purchase_max") == page - 1) {
                int slot = index % MMGlobalExchanges.instance.getConfig().getInt("black_market_personal_purchase_max");
    ...
    public int getBuyNum() {
        return playerData.getBlackBuyItems().size();
    }
    ...
    private void addInfo(Player player) {
        ...
        lore.add(String.valueOf((playerData.getBlackBuyItems().size() - 1) / MMGlobalExchanges.instance.getConfig().getInt("black_market_personal_purchase_max") + 1));
        itemMeta.setLore(lore);
        item.setItemMeta(itemMeta);
        addItem(52, item);
    }
