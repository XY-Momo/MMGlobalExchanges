...
public class Search extends Chest {
    ...
    private void init(Player player) {
        ...
        if (!MMGlobalExchanges.instance.getConfig().getBoolean("use_dragoncore")) {
            ...
            if (page == 1) {
                ...
                this.removeItem(50);
                this.removeItem(51);
                if (this.size > MMGlobalExchanges.instance.getConfig().getInt("market_search_max")) {
                    this.addNextPage();
                    this.addLastPage();
                }
            } else if (page < (this.size - 1) / MMGlobalExchanges.instance.getConfig().getInt("market_search_max") + 1) {
                this.addFirstPage();
    ...
    private void loadItems(Player player) {
        List<ItemStack> items = MMGlobalExchanges.marketFile.loadItems();
        ...
        for (Map.Entry<List<String>, Integer> entry : itemStacks.entrySet()) {
            ...
            // 一页多少物品
            if (index / MMGlobalExchanges.instance.getConfig().getInt("market_search_max") == page - 1) {
                int slot = index % MMGlobalExchanges.instance.getConfig().getInt("market_search_max");
