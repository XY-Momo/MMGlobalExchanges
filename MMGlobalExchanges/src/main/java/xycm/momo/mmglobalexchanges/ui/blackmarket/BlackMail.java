 ...
 * 个人邮箱
 */
public class BlackMail extends Chest {
    ...
    private void init(Player player) {
        ...
        int max_item = MMGlobalExchanges.instance.getConfig().getInt("black_market_personal_mail_max");
        this.addReturn();
        if (page == 1) {
            this.removeItem(47);
            this.removeItem(48);
            this.removeItem(50);
            this.removeItem(51);
            if (playerData.getBlackMailItems().size() > max_item) {
                this.addNextPage();
                this.addLastPage();
            }
        } else if (page < (playerData.getBlackMailItems().size() - 1) / max_item + 1) {
            this.addFirstPage();
    ...
    public void pickAll(Player player) {
        MMGlobalExchanges.blackMail.setPage(player, 0);
        MMGlobalExchanges.blackMail.open(player);
        History history = new History("BlackMarket");
        long sum = 0;
        for (int id : playerData.getBlackMailItems()) {
            sum += history.getPrice(id);
        ...
        if (MMGlobalExchanges.instance.getConfig().getBoolean("info")) {
            MMGlobalExchanges.blackMail.setInfo(player, "共收入 " + sum + " 时息");
            MMGlobalExchanges.blackMail.setPage(player, 0);
            MMGlobalExchanges.blackMail.open(player);
        } else {
            MMGlobalExchanges.blackMail.setPage(player, 0);
        ...
        playerData.addBlackSellPrice(sum);
    }
    ...
    private void loadItems(Player player) {
        ...
        History history = new History("BlackMarket");
        for (int id : playerData.getBlackMailItems()) {
            if (index / MMGlobalExchanges.instance.getConfig().getInt("black_market_personal_mail_max") == page - 1) {
                int slot = index % MMGlobalExchanges.instance.getConfig().getInt("black_market_personal_mail_max");
                ItemStack itemStack = history.getItem(id);
                ItemMeta itemMeta = itemStack.getItemMeta();
                List<String> lore = itemMeta.hasLore() ? itemMeta.getLore() : new ArrayList<>();
                lore.add("§f商品唯一ID: " + id);
                lore.add("§f收入: " + Math.round(history.getPrice(id)) + " 时息");
    ...
    public int getMailNum() {
        return playerData.getBlackMailItems().size();
    }
    ...
    private void addInfo(Player player) {
        ...
        lore.add(String.valueOf((playerData.getBlackMailItems().size() - 1) / MMGlobalExchanges.instance.getConfig().getInt("black_market_personal_mail_max") + 1));
        itemMeta.setLore(lore);
        item.setItemMeta(itemMeta);
        addItem(52, item);
    }
