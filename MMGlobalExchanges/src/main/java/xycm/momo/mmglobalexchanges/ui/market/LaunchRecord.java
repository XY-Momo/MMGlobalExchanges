 ...
 * 个人上架记录
 */
public class LaunchRecord extends Chest {
    ...
    public LaunchRecord(String title) {
        super(title, 54);
    }
    ...
    private void init(Player player) {
        ...
        int max_item = MMGlobalExchanges.instance.getConfig().getInt("market_personal_launch_max");
        this.addLaunch();
        this.addReturn();
        if (page == 1) {
            this.removeItem(47);
            ...
            this.removeItem(50);
            this.removeItem(51);
            if (playerData.getLaunchItems().size() > max_item) {
                this.addNextPage();
                this.addLastPage();
            }
        } else if (page < (playerData.getLaunchItems().size() - 1) / max_item + 1) {
            this.addFirstPage();
    ...
    private void loadItems(Player player) {
        ...
        for (int id : playerData.getLaunchItems()) {
            if (index / MMGlobalExchanges.instance.getConfig().getInt("market_personal_launch_max") == page - 1) {
                int slot = index % MMGlobalExchanges.instance.getConfig().getInt("market_personal_launch_max");
    ...
    public int getLaunchNum() {
        return playerData.getLaunchItems().size();
    }
    ...
    private void addInfo(Player player) {
        ...
        lore.add(String.valueOf((playerData.getLaunchItems().size() - 1) / MMGlobalExchanges.instance.getConfig().getInt("market_personal_launch_max") + 1));
        itemMeta.setLore(lore);
        item.setItemMeta(itemMeta);
        addItem(52, item);
    }
